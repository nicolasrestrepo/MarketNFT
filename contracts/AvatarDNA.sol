...
contract AvatarDNA {
    ...
    // this pseudo ramdon function is deterministic and should no be use on production
    // for production we can use chainlink
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _mintAddress) 
        public 
        pure
        returns(uint256)
    {
        uint256 combinedParams = _tokenId + uint160(_mintAddress);
